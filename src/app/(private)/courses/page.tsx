...
export default async function CoursesPage({ searchParams }: CoursesPageProps) {
  ...
  return <>
    ...
    <div className="two-col"><section className="panel"><div className="panel-title"><h2>Библиотека курсов</h2><span className="count-badge">{courses.length}</span></div>{courses.length ? <div className="card-list">{courses.map((c) => <article className="course-card course-card-manage" key={c.id}><div className="course-icon large"><BookMarked/></div><div><strong>{c.title}</strong><span>{c.grade ? `${c.grade} класс` : 'класс не указан'}</span></div><span className="soft-badge">Рабочий</span><details className="course-manage"><summary><Pencil size={14}/>Редактировать курс</summary><form action={updateCourse} className="course-edit-form"><input type="hidden" name="courseId" value={c.id}/><label>Название<input name="title" required defaultValue={c.title}/></label><label>Класс<input name="grade" type="number" min="1" max="11" defaultValue={c.grade ?? ''} placeholder="Не указан"/></label><button className="course-action save" type="submit">Сохранить</button></form><div className="course-actions"><CourseDeleteButton courseId={c.id} title={c.title}/></div></details></article>)}</div> : <EmptyState title="Курсы добавляются постепенно" text="Не нужно знать всю базу учебников заранее."/>}</section>
