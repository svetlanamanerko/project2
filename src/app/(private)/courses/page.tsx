...
export default async function CoursesPage() {
  ...
  return <>
    ...
    <div className="two-col"><section className="panel"><div className="panel-title"><h2>Библиотека курсов</h2><span className="count-badge">{courses.length}</span></div>{courses.length ? <div className="card-list">{courses.map((c) => <article className="course-card" key={c.id}><div className="course-icon large"><BookMarked/></div><div><strong>{c.title}</strong><span>{c.grade ? `${c.grade} класс` : 'класс не указан'}</span></div><span className="soft-badge">Рабочий</span></article>)}</div> : <EmptyState title="Курсы добавляются постепенно" text="Не нужно знать всю базу учебников заранее."/>}</section>
