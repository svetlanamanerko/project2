...
const typeLabels: Record<LessonExercise['type'], string> = {
  ...
  speaking: 'SPEAKING CHALLENGE', oral_drill: 'ORAL PRACTICE', self_check: 'SELF-CHECK', reading: 'READING SOURCE', listening: 'LISTENING SOURCE',
};
...
function TaskBody({ exercise, value, feedback, update, openSource }: {
  ...
  if (exercise.type === 'oral_drill') {
    const modeLabels = { say: 'SAY', read_aloud: 'READ ALOUD', say_and_spell: 'SAY & SPELL', repeat: 'REPEAT', word_chain: 'WORD CHAIN', quick_name: 'QUICK NAME' } as const;
    return <div className={styles.oralDrill}><strong>{modeLabels[exercise.mode || 'say']}</strong><p>{exercise.instruction}</p><div>{exercise.items.map((item, index) => <article key={item.id}><b>{index + 1}</b><span>{item.label}</span></article>)}</div></div>;
  }
  if (exercise.type === 'self_check') return <div className={styles.selfCheck}>{exercise.items.map((item) => {
    const checked = value[item.id] === 'checked';
    return <button type="button" key={item.id} className={checked ? styles.checkedChip : ''} aria-pressed={checked} onClick={() => { const next = { ...value }; if (checked) delete next[item.id]; else next[item.id] = 'checked'; update(next); }}><span aria-hidden="true">{checked ? '✓' : '○'}</span>{item.label}</button>;
  })}</div>;
...
export function LessonPlayerV3(props: Props) {
  ...
  const setAnswer=(value:Record<string,string>)=>{if(exercise){setResponses((current)=>({...current,[exercise.id]:value}));if(exercise.type==='self_check'&&exercise.items.every((item)=>value[item.id]==='checked'))markComplete(exercise.id);}};
  ...
  const openSource=()=>{if(!exercise?.resourceId)return;setSourceId(exercise.resourceId);setSourceMode(sourceMode==='closed'?'floating':sourceMode);};
  ...
  return <div className={`${styles.shell} ${fullscreen?styles.fullscreen:''} ${sourceMode==='docked'?styles.withSource:''}`} data-theme={props.designStyle||'teen-study'} data-lesson-player="v3">
    <span className={styles.versionMarker} aria-label="Lesson Player V3">V3</span>
